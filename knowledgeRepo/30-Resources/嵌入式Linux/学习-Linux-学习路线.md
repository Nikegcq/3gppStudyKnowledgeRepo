...
- [ ] G1：完成阶段 1+2，能在 QEMU+GDB 里断住自己写的 `open/read`，并解释用户态 `read()` 到驱动回调的路径。
...
- [ ] T1 最小模块闭环：写 hello 模块 + Makefile，走通 `insmod` → `lsmod` → `dmesg` → `rmmod`；读 `include/linux/module.h`，理解 `module_init/exit` 不是 `main`。
- [ ] T2 字符 / 杂项设备：读 `include/linux/fs.h` 的 `struct file_operations` 与 `fs/read_write.c` 的 `vfs_read`；用 `class_create/device_create` 自动建节点；练 `copy_to_user/copy_from_user`、`private_data + container_of`、mutex 保护；对照 `drivers/char/misc.c`。
...
- [ ] 执行版阶段 1：最小驱动闭环（hello 模块 + misc 字符设备 + VFS 调用链）
